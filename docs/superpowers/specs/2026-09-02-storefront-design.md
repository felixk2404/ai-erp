...
| בוט | כפתור צף בפינת ה-end התחתונה (שמאל ב-RTL), כדי לא להסתיר את פס העובדות של ה-hero ולא להתנגש במגירת העגלה, פותח פאנל צ'אט. הודעת פתיחה מותאמת לעמוד ("שאלות על TY-HP-200?" בעמוד מוצר). כרטיסי מוצר בתשובות עם "הוסף לסל" ישירות מהצ'אט. Rate limit 20/דקה ל-IP (מועתק מהניהול). |
