...
| מלאי | שדה `Products.Stock` (מספר). קטגוריה "שירותים" = ללא מלאי, זמין תמיד | דרישת הסטודנט: כמויות רק לפריטים פיזיים |
...
| `/products` | קטלוג מלא: סינון לפי קטגוריה (pills), חיפוש חי (client, על 34 פריטים), מיון (מחיר/שם), מצב "רשת" ו"מפרט" (טבלה מונוספייס). כרטיס: תמונה, שם, 1 שורת מפרט, מחיר בתוך כפתור "הוסף לסל — 349 ₪", תג מלאי ("נותרו 3", "נגמר → הודיעו לי"). |
