...
- `--input` rgba(255,255,255,.40) — the field border is decoupled from `--rule-strong` on purpose: it is the only mark of where to type, so it needs 3:1 (WCAG 1.4.11), while the decorative hairline does not.
...
- Inputs — `bg-well`, border `--input`, focus border signal + ring signal/25. Native selects/textarea also `bg-well`. `<Input name>` inside `EntityDialog` picks up `aria-invalid`/`aria-describedby` from `FormErrorsContext` on its own.
- Tables — wrapper `panel overflow-hidden`; header 11px tracked readout-3 on `chassis-2/60`; rows hover `signal-soft/40`. `<Table label="…">` is required: it names the focusable `role="region"` scroll container (so off-screen columns are keyboard-reachable) and emits the `sr-only` caption.
...
- SystemMap — SVG 440×300 **from `sm` up only**; below it the labels collide and the list below carries the same information. Hub r30 center, 12 nodes on ellipse rx172/ry112; `.packet` dash animation on traces for nodes with runs; labels: right-side nodes `text-anchor=end`, left `start` (RTL SVG). Legend list below (identity never color-alone).
...
- ProductCard — `Tilt` (≤5°, spring, cyan glare) around `panel`; image on chassis-2 with inset ring white/10 + bottom fade; SKU chip mono cyan 11px on void/90 (opaque enough to survive a white product shot behind it).
...
- Hotkeys — `?` help dialog, `g`+`d/i/l/c/p/t` navigate, `n` clicks `[data-hotkey="new"]` (EntityDialog + NewInvoiceDialog triggers). Inactive while typing or a dialog is open, and switchable off from the help dialog itself (`localStorage erp-hotkeys`, WCAG 2.1.4). `HotkeysHelpButton` in the sidebar opens the same dialog without the keyboard.
...
- Focus & pending — global `:focus-visible` = 2px `--signal` outline, offset 2 (10.87:1 on void). Write actions never use `disabled`: `aria-disabled` + an in-handler guard + an `sr-only` live region, so the focused control stays under the user's finger for the whole round trip.
- Motion — Reveal 8px/320ms/50ms stagger; CountUp 600ms (skipped entirely under reduced motion — `MotionConfig` does not reach the imperative `animate()`); View Transitions unchanged; `MotionConfig reducedMotion="user"`. Everything < 300ms per element, transform/opacity only; beams/aurora are background and stop under reduced-motion.
