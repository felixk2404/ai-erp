...
export function ThinkingPanel() {
  ...
  return (
    // האזור החי יושב על תווית השלב בלבד; השעון מחוץ לו, אחרת קורא המסך מקריא את הפאנל כל שנייה (4.1.3)
    <div className="mt-4 flex-1 flex flex-col" aria-busy="true">
      <div className="relative overflow-hidden rounded-md border border-rule bg-well/60 p-3 flex-1 min-h-[112px]">
        ...
        <div className="space-y-2.5">
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s} className={`flex items-center gap-2.5 text-xs ${state === 'todo' ? 'text-readout-3' : state === 'done' ? 'text-readout-2' : 'text-readout'}`}>
      ...
      <div className="mt-3 flex items-center justify-between text-[12px] text-readout-3">
        ...
        <span className="mono" dir="ltr" aria-hidden>
