...
export function Timeline({ steps }: { steps: TimelineStep[] }) {
  return (
    <ol className="flex items-start gap-0" aria-label="ציר זמן">
      {steps.map((s, i) => (
        <li key={s.label} className="flex-1 relative" aria-current={s.state === 'current' ? 'step' : undefined}>
          ...
          <div className="relative flex flex-col items-center text-center gap-2">
            ...
            <div>
              <div className={`text-xs font-medium ${s.state === 'todo' ? 'text-ink-3' : 'text-ink'}`}>
                {s.label}
