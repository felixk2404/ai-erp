...
export function StatusBar({ rows }: { rows: StatusRow[] }) {
  ...
  if (total === 0) return <p className="text-sm text-ink-3">אין חשבוניות עדיין.</p>;
  return (
    <div>
      <div className="flex h-3 w-full gap-0.5 rounded-full overflow-hidden" role="img" aria-label={`חלוקת ${total} חשבוניות לפי סטטוס`}>
        {shown.map((r) => (
          <div key={r.status} className={`${FILL[r.status]} h-full`} style={{ width: `${(r.count / total) * 100}%` }} title={`${statusMeta('Invoices', r.status).label}: ${r.count}`} />
        ))}
      </div>
        ...
        {shown.map((r) => (
          <li key={r.status} className="flex items-center justify-between gap-2">
            ...
            <span className="num text-ink">
              {r.count} · <Money value={r.total} className="text-ink-2" />
            </span>
          </li>
        ))}
      </ul>
