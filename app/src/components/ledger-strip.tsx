...
function DeltaChip({ d }: { d: Delta }) {
  if (d.pct === null) return <span className="mono text-[11px] text-readout-3">— אין השוואה</span>;
  const up = d.pct >= 0;
  return (
    <span className={`inline-flex items-center gap-1 mono text-[11px] ${up ? 'text-led-green' : 'text-led-red'}`} title="מול החודש הקודם">
      ...
      <div className="relative grid grid-cols-2 md:flex md:divide-x md:divide-x-reverse md:divide-rule">
        {items.map((it) => (
          <div key={it.label} className={`px-5 py-4 md:px-6 md:py-5 ${it.hero ? 'col-span-2 md:flex-[1.7] border-b border-rule md:border-b-0' : 'md:flex-1 odd:border-e odd:border-rule md:odd:border-e-0'}`}>
            <div className="text-[11px] font-medium tracking-wide text-readout-3">{it.label}</div>
